...
func (s *ServiceLogger) Println(v ...interface{}) {
	if s.enabled {
		s.logger.Infof("SHENXIAN@%s>> %v", time.Now().Format(time.DateTime), fmt.Sprint(v...))
	}
}

func (s *ServiceLogger) Printf(format string, v ...interface{}) {
	if s.enabled {
		s.logger.Infof("SHENXIAN@%s>> "+format, append([]interface{}{time.Now().Format(time.DateTime)}, v...)...)
	}
}

func (s *ServiceLogger) Error(v ...interface{}) error {
	if s.enabled {
		return s.logger.Errorf("SHENXIAN@%s>> %v", time.Now().Format(time.DateTime), fmt.Sprint(v...))
	}
	return nil
}
...
func (s *ServiceLogger) Errorf(format string, v ...interface{}) error {
	if s.enabled {
		return s.logger.Errorf("SHENXIAN@%s>> "+format, append([]interface{}{time.Now().Format(time.DateTime)}, v...)...)
	}
	return nil
}
