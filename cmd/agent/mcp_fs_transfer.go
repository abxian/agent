...
type serialIOStreamSender struct {
	mu     sync.Mutex
	stream pb.ShenxianService_IOStreamClient
}

func newSerialIOStreamSender(stream pb.ShenxianService_IOStreamClient) *serialIOStreamSender {
...
// pb.ShenxianService_IOStreamClient 的接口工作，但把 Send 全部走串行化器；
// CloseSend / Recv 等非写入侧方法仍直通底层 stream。
type streamWithSerialSender struct {
	pb.ShenxianService_IOStreamClient
	Stream pb.ShenxianService_IOStreamClient
...
func runFsTransferOnStream(stream pb.ShenxianService_IOStreamClient, earlyErr string, req *model.FsTransferRequest) {
	if earlyErr != "" {
...
// 半截内容覆盖目标文件。
func fsTransferUpload(stream pb.ShenxianService_IOStreamClient, req *model.FsTransferRequest) {
...
func fsTransferDownload(stream pb.ShenxianService_IOStreamClient, req *model.FsTransferRequest) {
	clean, err := resolveFsPath(req.Path)
	if err != nil {
		sendXferErr(stream, "invalid path: "+err.Error())
		return
...
func sendXferFixedHeader(stream pb.ShenxianService_IOStreamClient, magic []byte, size uint64, hash []byte) error {
	var buf bytes.Buffer
	buf.Write(magic)
...
func sendXferOK(stream pb.ShenxianService_IOStreamClient, sha string, size int64) error {
...
func sendXferErr(stream pb.ShenxianService_IOStreamClient, msg string) {
	var buf bytes.Buffer
